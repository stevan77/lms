...
const submit = () => {
    form.post(route('teacher.lessons.store', props.subchapter.id));
};
</script>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">{{ t('Create Lesson') }}</h1>
                    <p class="mt-1 text-sm text-gray-500">{{ course.name }} → {{ chapter.title }} → {{ subchapter.title }}</p>
                </div>
                <button
                    type="button"
